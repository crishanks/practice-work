function nbaCup(resultSheet, toFind) {
    ...
    const gamesArray = resultSheet.split(',');
    for (let i = 0; i < gamesArray.length; i++) {
      let teamOne = [];
      let teamTwo = [];
      let scoreOne = 0;
      let scoreTwo = 0;
      const parts = gamesArray[i].split(' ');
      let teamOneDone = false;
  
      for (let j = 0; j < parts.length; j++) {
        const currentValue = parts[j];
        if (isNaN(currentValue)) {
          if (teamOneDone) {
            teamTwo.push(currentValue);
          } else {
            teamOne.push(currentValue);
          }
        } else {
          if (scoreOne === 0) {
            scoreOne = +currentValue;
            teamOneDone = true;
          } else {
            scoreTwo = +currentValue;
          }
        }
      }
